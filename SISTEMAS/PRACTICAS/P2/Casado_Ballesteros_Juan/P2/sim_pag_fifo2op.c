...
void referenciar_pagina (ssistema * S, int pagina, char op)
{
	...
	//FIFO2op
	S->tdp[pagina].referenciada = 1;				//Digo que he referenciado la página
}
...
int elegir_pagina_para_reemplazo (ssistema * S)
{
	...
	//FIFO
	marco = S-> tdm[S->listaocupados].sig;			//Me sitúo en el primero elemento de la lista
	//FIFO 2 OP
	while (S->tdp[S->tdm[marco].pagina].referenciada)	//Busaco la primera página no referenciada
	{
		S->tdp[S->tdm[marco].pagina].referenciada = 0;
    ...
    if (S->detallado)
        printf ("@ Eligiendo (fifo2op) P%d de M%d para "
                "reemplazarla\n", victima, marco);
    ...
    for (m=0; m< S->nummarcos; m++)
    {
        ...
        if (p==-1)
            ...
            printf ("%8d   %8d   %6d     %6d    %6d\n", m, p,
                    S->tdp[p].presente, S->tdp[p].modificada, S->tdp[p].referenciada);		//Muestro el bit de refenciada
        else
            printf ("%8d   %8d   %6d     %6s   %6s¡ERROR!\n", m, p,
                    S->tdp[p].presente, "-","-");
    }
}
...
void mostrar_informe_reemplazo (ssistema * S)
{
	...
	{											//Primero muestro todos menos el último
		printf("Pagina: %6d Marco: %6d Referenciada: %6d\n",S->tdm[presente].pagina, presente,S->tdp[S->tdm[presente].pagina].referenciada);//Muestro el bit de referenciada
		presente = S->tdm[presente].sig;
	}

	//Muestro el último marco que es el que me faltaba
	printf("Pagina: %6d Marco: %6d Referenciada: %6d\n",S->tdm[presente].pagina, presente,S->tdp[S->tdm[presente].pagina].referenciada);
    printf ("Reemplazo fifo de segunda oportunidad"
